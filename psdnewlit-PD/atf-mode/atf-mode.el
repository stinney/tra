...
;    (define-key atf-mode-map "C" 'atf-SZ)
    ...
    (define-key atf-mode-map "\C-c\C-p" 'atf-pdf)
...
After you call the checker you will have two windows; one with your
text, the other with the errors (if any).  Use the key sequence
Ctrl-x-` (control-x then backtick) to jump to the first error in your
ATF file, and then repeat this to step through the errors.

For the ATF documentation see URL `http://cdl.museum.upenn.edu/doc/ATF/'."
  (interactive "P")
  (let ((on-p atf-mode))
    ...
    (when atf-mode
      ...
      (easy-menu-define atf-mode-menu atf-mode-map "" 
			      ...
			      ["PDF of buffer" atf-pdf t]
			      ["Update Unicode" atf-update-unicode t]
...
(defun atf-check-rpc-async(data name)
  (interactive)
  (xml-rpc-method-call "http://cdl.museum.upenn.edu/cgi-bin/rpc.plx"
		       'cdl.atf.check
		       `(("data" . ,data)
			 ("atf-file-name" . ,name))))

(defun atf-check-rpc(data name)
  (interactive)
  (let ((inhibit-read-only t))
    (insert
     (cdr 
...
(defun atf-check () ; based on 'compile'
  (interactive)
  (save-some-buffers (not compilation-ask-about-save) nil)
  (atf-check-start 
   (buffer-substring-no-properties (point-min) (point-max))
...
(defun atf-check-start (data name)
  (setq mode 'compilation-mode)
  (let* ((name-of-mode
	  (if (eq mode t)
	      (prog1 "compilation" (require 'comint))
...
(defun atf-lemmatize ()
  ...
  (let* ((bufstr (buffer-substring-no-properties 
		  (point-min) (point-max)))
	 (name (buffer-name))
	 (resp (xml-rpc-method-call "http://cdl.museum.upenn.edu/cgi-bin/rpc.plx"
				    'cdl.atf.lemmatize
				    `(("data" . ,bufstr)
				      ("atf-file-name" . ,name)))))
...
(defun atf-rpc-errmsg (res)
  (cadr (car (cdr (car resp)))))

(defun atf-rpc-resval (res)
  (cdr (car (cdr res))))

(defun atf-update-unicode ()
  "Update the Unicode characters used in the buffer.

The Unicode conventions used for ATF/Unicode have one known
bug (effective in files created before the 04-21-2008 version of
cuneitex.el).  Also, new characters added in Unicode 5.x include
subscript-x and real half square brackets.  This function edits
the current buffer to fix the bug; a future version of the
function will also upgrade subscript-+ to subscript-x and replace
old half-bracket hacks with the correct Unicode characters."
  (interactive)
  (save-excursion
    (perform-replace "˒" "ʾ" nil nil nil nil nil (point-min) (point-max))
    (perform-replace "₊" "ₓ" nil nil nil nil nil (point-min) (point-max))
...
(defun atf-browse-doc ()
  (interactive)
  (browse-url "http://cdl.museum.upenn.edu/doc/ATF/"))

(defun atf-version ()
  "Display the release-version of the ATF mode you are using"
  (interactive)
  (message "ATF mode version 2008-04-21")
)
